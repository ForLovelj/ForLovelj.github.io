...
这一类就是在path中添加一个基本形状，基本形状部分和前面cavans所讲的绘制基本形状并无太大差别，可以添加圆、椭圆、矩形和圆角矩形等，本次只将其中不同的部分摘出来详细讲解一下。
...
上面我们已经讲过这两种规则下判断图形内部的方式（内部才需要填充），通常情况下这两种方法的判断结果是相同的，但是自相交图形则不一定，例如上面的例子或者下图：

> 自相交图形定义：多边形在平面内除顶点外还有其他公共点。

> ![相交图形的奇偶和非零规则填充模式](https://ForLovelj.github.io/img/相交图形的奇偶和非零规则填充模式.png)

前面讲添加子图形时说过 `Path.Direction` 参数中顺时针和逆时针会对图形的渲染结果有影响，从上面的图中可以出来，**图形边的方向会对非零奇偶环绕数规则填充结果有影响**，继而会影响渲染的结果。
